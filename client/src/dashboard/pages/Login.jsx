...
const Login = () => {
  ...
  const submit = async (e) => {
    ...
    try {
      setLoader(true);
      const data = await loginUser(state); // Call the loginUser function
      console.log(data);
      setLoader(false);
      ...
      console.log(error);
      setLoader(false);
      toast.error(error.response?.data.message || 'Login failed. Please try again.'); // Updated error handling
    }
  };
      ...
      <div className='w-[340px] text-slate-600 shadow-md'>
        <div className='bg-white h-full px-7 py-8 rounded-md'>
          ...
          <form onSubmit={submit}>
            <div className='grid grid-col-2 gap-x-8 mb-3'>
...
//             <div className='flex flex-col gap-y-2'>
//               <label className='text-md font-medium text-gray-600' htmlFor="email">Email</label>
//               <input value={state.email} required onChange={inputHandle} type='email' placeholder='email' name='email' className='px-3 py-2 rounded-md 
//                   outline-0 border border-gray-300 focus:border-green-500 h-10' id='email'/>
//             </div>
//             <div className='flex flex-col gap-y-2'>
//               <label className='text-md font-medium text-gray-600' htmlFor="password">Password</label>
//               <input onChange={inputHandle} required value={state.password} type="password" placeholder='password' name='password' className='px-3 py-2 rounded-md 
